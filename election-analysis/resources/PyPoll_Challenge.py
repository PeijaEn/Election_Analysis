...
with open(file_to_save, "w") as txt_file:
    ...
    for countyNames in countyVotes:
        ...
        county_votes = countyVotes[countyNames]
        
        # 6c: Calculate the percentage of votes for the county.
        vote_percentage = float(county_votes) / float(total_votes) * 100

         # 6d: Print the county results to the terminal.
        county_results = (f"{countyNames}: {vote_percentage:.1f}% ({county_votes:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_votes > winningCountVote) and (vote_percentage > winning_percentage):
            winningCounty = countyNames
            winningCountVote = county_votes
